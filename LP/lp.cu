...
int m_numMaxBatches = 0; //number of LPs
...
void lplibInit(int numMaxConstraints, int numMaxBatches)
{
    ...
    m_numMaxBatches = numMaxBatches;
    m_numMaxConstraints = numMaxConstraints;
...
{
	if(numBatches > m_numMaxBatches)
	{
		throw std::invalid_argument("Num batches higher than maximum number of batches");
	}

	//------------------------------------------
	//Initialize kernel
	int blockSize = BlockDimSize;
	int gridSize = (m_numMaxBatches + blockSize - 1) / blockSize;
	dim3 b, g;
	b.x = blockSize;
	g.x = gridSize;
...
{
	int batches = numBatches; //number of LPs
	...
	if(!parseBenchmark(fileName, &constraintsSingle, &optimiseSingle, &size)){
		return 1;
	}
	printf("Done\n");
	...
	//------------------------------------------
	//Initialise library, allocate memory
	lplibInit(size, batches);
	...
	constraints = lplibGetConstraints();
	optimise = lplibGetOptimise();
	for (int i = 0; i < batches; i++) {
		for (int j = 0; j < size; j++) {
			float4 vx = constraintsSingle[j];
	...
	output = lplibGetOutput();
	for (int i = 0; i < batches; i++) {
		printf("Batch %i \t Target x:%f y:%f \t Optimal location is x: %f y: %f \t value of %f\n", i,optimise[i].x, optimise[i].y, output[i].x, output[i].y, glm::dot(optimise[i], output[i]) );
	}
